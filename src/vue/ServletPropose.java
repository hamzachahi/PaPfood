			...
			if (action != null) {
				...
				if (action.equals("afficherSousVendables")) {
					...
					if (total <= 0 || total == null) {
						...
						listProdSer = new ArrayList<>();
						listProdSer.addAll(productDao.findAllProductByIdProvider(utilisateur.getId(), end, begin));
						listProdSer.addAll(serviceDao.findAllServiceByIdProvider(utilisateur.getId(), end, begin));
				...
				if (action.equals("addSalable")) {
					begin = (long) session.getAttribute("beginpr");
					end = (long) session.getAttribute("endpr");
				...
				if (action.equals("removeSalable")) {
					...
					// listProdSer.addAll(productDao.findAllProductByIdProvider(utilisateur.getId(),
					// end, begin));
					// listProdSer.addAll(serviceDao.findAllServiceByIdProvider(utilisateur.getId(),
					// end, begin));
					// request.setAttribute("listeDeSalable", listProdSer);
				}
